...
def cambios(x):#cambia espacios por guin bajo y mayusculas por minusculas
    nueva_lista=[]
    
    for i in x:
        if type(i)==type([]):
            nueva_lista.append(cambios(i))
        else:
            a=i.split()
            b='_'.join(a)
            nueva_lista.append(b.lower())
            print('el nombre original del archivo es:\n'+i+'   y es cambiado por:     '+b.lower()+'\n')
    return nueva_lista



a=carpeta('C:\\Users\\Sorek\\Desktop\\d')  #colocar la dirección de la carpeta que desea investigar
